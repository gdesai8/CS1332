...
public class StringSearching {
    ...
    public static List<Integer> kmp(CharSequence pattern, CharSequence text) {
        if (pattern == null || pattern.length() == 0) {
            throw new IllegalArgumentException("The pattern is either null or "
                    + "of length 0.");
        }
        if (text == null) {
        ...
        while (i <= (text.length() - pattern.length())) {
            ...
            if (j == 0) {
                ...
                int alignment = failureTable[j - 1];
                i = i + j - alignment;
                j = alignment;
            }
        }
    ...
    public static int[] buildFailureTable(CharSequence pattern) {
        if (pattern == null) {
            throw new IllegalArgumentException("Cannot make a failure table fo"
                    + "r a null pattern.");
        }
    ...
    public static List<Integer> boyerMoore(CharSequence pattern,
        ...
        while (i <= (text.length() - pattern.length())) {
            ...
            while (j >= 0 && (current = text.charAt(i + j)) == pattern.charAt(j)) {
                j = j - 1;
            }
            ...
            if (j == -1) {
                ...
                int shift = lastTable.getOrDefault(current, -1);
                if (shift < j) {
                    i = i + (j - shift);
                } else {
                    i = i + 1;
    ...
    public static int generateHash(CharSequence current, int length) {
        if (current == null) {
            throw new IllegalArgumentException("The sequence to be hashed is n"
                    + "ull.");
        }
        if (length < 0 || length > current.length()) {
            throw new IllegalArgumentException("The length of the hash is wron"
                    + "g.");
        }
        int hash = 0;
        for (int i = 0; i < length; i++) {
            hash = hash + ((int) current.charAt(i) * pow(BASE, length - 1 - i));
        }
        return hash;
    }
    ...
    public static int updateHash(int oldHash, int length, char oldChar,
            char newChar) {
        oldHash = (oldHash - ((int) oldChar * pow(BASE, length - 1))) * BASE + (int) newChar;
        return oldHash;
    }
